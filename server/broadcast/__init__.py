...
def broadcast_user_status_change(user_id, is_online):
    """通知一个用户的所有好友其状态变化。"""
    from server.util.database import get_all_friends, get_user_by_id
    ...
    for friend in friends:
        friend_id = friend['id']
        sc = memory.user_id_to_sc.get(friend_id) # 检查好友是否在线
        if sc:
            try:
                sc.send(MessageType.user_status_change, {
                    'user_id': user_id,
                    'username': user_info['username'],
                    'online': is_online
                })
            except Exception as e:
                print(f"通知好友 {friend_id} 状态变化时出错: {e}")